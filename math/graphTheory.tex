...
\newpage
\subsection{Results}
\begin{proposition}[Graph movement is symmetric digraph movement]\label{prop1}
...
\begin{proof}
	...
	Let $V_1 = V_0$. Let $(u,v),(v,u) \in E_1$ for all $\{u,v\} \in
	E_0$. Then since $(u,v) \in E_1$ implies $(v,u)\in E_1$,
...
\begin{proposition}[Composition is partial mapping]\label{prop2}
	Let $\mathcal{M} = (M,+)$ be a movement structure. Let $x,y \in M$, then if
	for some $z,z' \in M$ we have $x+y=z$ and $x+y=z'$ then $z=z'$.
\end{proposition}
\begin{proof}
...
\begin{proposition}[Composition is assosiative]\label{prop3}
	Let $\mathcal{M} = (M,+)$ be a movement structure. Then for all $x,y,z \in
	M$, $x+(y+z) = (x+y)+z$ if $\pi_2(x)= \pi_1(y)$ and $\pi_2(y)= \pi_1(z)$,
	that is if $x+y$ and $y+z$ is defined.
\end{proposition}
\begin{proof}
...
\begin{proposition}[Composition has left and right 
	per element identity]\label{prop4}
	Let $\mathcal{M} = (M,+)$ be a movement structure. Then for all $x \in M$
	there exists $e_l\in M$ and $e_r\in M$ such that $e_l+x=a$ and $x+e_r = a$.
\end{proposition}
\begin{proof}
	Let $e_l = \pi_1(x)\pi_1(x)$. Then $e_l \in M$ since by Definition~\ref{def
	graph movement} $vv\in M$ for all $v\in V$. So we have
	\begin{equation}
	\begin{aligned}
